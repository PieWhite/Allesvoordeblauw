...
type Aggregator struct {
	...
	packetSizes     map[string]int             // src_ip → count of flows with bytes
	totalBytes      map[string]int64           // src_ip → sum of in_bytes across all flows
...
func (a *Aggregator) Update(record NetflowRecord) {
	...
	if record.InBytes > 0 {
		a.packetSizes[srcIP]++
		a.totalBytes[srcIP] += record.InBytes
	}
}
...
// PacketSizeFlows returns the count of flows with bytes for a source IP.
func (a *Aggregator) PacketSizeFlows(srcIP string) int {
	return a.packetSizes[srcIP]
}

// TotalBytes returns the total in_bytes sum for a source IP.
func (a *Aggregator) TotalBytes(srcIP string) int64 {
	return a.totalBytes[srcIP]
}
