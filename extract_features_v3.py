...
def extract_features_v2(input_path, output_path, window_size='5min'):
    ...
    # V3: Small packets filter (<= 3 packets)
    df['is_small_packet'] = (df['in_packets'] <= 3).astype(int)
    ...
    # Aggregation mapping
    agg_funcs = {
        'first': ['count', 'min'],        # flow_count, window_start
        'last': 'max',                    # window_end
        'dst4_addr': 'nunique',           # unique_dst_ips
        'dst_port': 'nunique',            # unique_dst_ports
        'in_bytes': ['sum', 'std'],       # total_bytes, bytes_std
        'in_packets': ['sum', 'std'],     # total_packets, packets_std
        'is_tcp': 'sum',
        'is_udp': 'sum',
        'is_icmp': 'sum',
        'syn_only': 'sum',               
    ...
    grouped = grouped.rename(columns={
        ...
        'is_small_packet_sum': 'count_small_packet',
    ...
    print("Calculating final ratios and percentages (V2/V3 Features)...")
    
    # Calculate Destination IP Entropy (V3)
    ip_counts = df.groupby(['src4_addr', 'time_window', 'dst4_addr']).size().reset_index(name='ip_c')
    ip_totals = ip_counts.groupby(['src4_addr', 'time_window'])['ip_c'].transform('sum')
    p = ip_counts['ip_c'] / ip_totals
    ip_counts['entropy_term'] = -p * np.log2(p)
    entropy_df = ip_counts.groupby(['src4_addr', 'time_window'])['entropy_term'].sum().reset_index(name='dst_ip_entropy')
    grouped = pd.merge(grouped, entropy_df, on=['src4_addr', 'time_window'], how='left')
    grouped['dst_ip_entropy'] = grouped['dst_ip_entropy'].fillna(0)
    ...
    # Fill NaNs for Standard Deviations (occurs when flow_count is 1)
    grouped['bytes_std'] = grouped['bytes_std'].fillna(0)
    grouped['packets_std'] = grouped['packets_std'].fillna(0)
    
    # Flow Rate (flows per second of active duration)
    grouped['window_active_duration'] = (grouped['window_end'] - grouped['window_start']).dt.total_seconds().clip(lower=1)
    grouped['flow_rate'] = grouped['flow_count'] / grouped['window_active_duration']
    
    # Small Packet Ratio
    grouped['pct_small_packet'] = (grouped['count_small_packet'] / flow_counts) * 100
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python extract_features_v3.py <input.json> <output.csv>")
        sys.exit(1)
